...
vec2 juliaBounds = vec2(windowSize.x * 0.4, windowSize.x * 0.4);

vec3 getColor(int iteration, int max_iteration, float x, float y) {
...
void main () {
  ...
  if ((gl_FragCoord.x-crosshairPos.x)*(gl_FragCoord.x-crosshairPos.x) + (gl_FragCoord.y-crosshairPos.y)*(gl_FragCoord.y-crosshairPos.y) < 100) && (gl_FragCoord.x == crosshairPos.x || gl_FragCoord.y == crosshairPos.y)) {
  ...
  } else if (showJulia && gl_FragCoord.x < juliaBounds.x && gl_FragCoord.y < juliaBounds.y) {
    
    float R = 2.5;
    float juliaMagnification = windowSize.y / (R * 2.0);
    
    vec2 pointerPos = trans/magnification;
    
    float zx = (gl_FragCoord.x  - windowSize.x * 0.2) / juliaMagnification;
    float zy = (gl_FragCoord.y - juliaBounds.y + windowSize.x * 0.2) / juliaMagnification;
    ...
    while (zx * zx + zy * zy < R * R && iteration < max_iteration) {
      ...
      zy = 2 * zx * zy + pointerPos.y;
      zx = xtemp + pointerPos.x;
    ...
    if (iteration == max_iteration) {
      ...
      vec3 c = getColor(iteration, max_iteration, zx, zy);
    ...
    if (iteration == max_iteration) {
      ...
      vec3 c = getColor(iteration, max_iteration, x, y);
      gl_FragColor = vec4(c, 1.0);
    }
  }
}
